...
module "dynamic_group" {
  ...
  tenancy_ocid   = var.tenancy_ocid
  compartment_id = var.compartment_id
  #DYNAMIC GROUP CONFIGURED IN TERRAFORM.TFVARS FILE (ANY NAME)
  dynamic_group_name = var.dynamic_group_name
  #POLICY NAME CONFIGURED IN TERRAFORM.TFVARS FILE (ANY NAME)
...
#waiting for a10lb process to come up
resource "time_sleep" "oci_instance_creation" {
  create_duration = var.delay_before_reboot

  triggers = {
    oci_public_ip_list = join(",", module.oci_compute.ip_list)
  }
}

resource "null_resource" "run_reboot_script" {
  depends_on = [time_sleep.oci_instance_creation]
  provisioner "local-exec" {
    command = "reboot-ha-pair.sh ${join(" ", module.oci_compute.ip_list)}"
  }
}
